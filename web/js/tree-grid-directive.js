app.run([
  ...
  function ($templateCache) {
    $templateCache.put(
      "/js/tree/temp/template.html",
      '<div class="container-fluid">\n' +
        '   <div class="col-md-12 text-right">\n' +
        '<a ui-sref="menu.addaccount" class=" btn btn-info">\n' +
        "<span>\n" +
        '<i class="icon ni ni-plus">\n' +
        "</i>\n" +
        "</span>\n" +
        "</a>\n" +
        "</div>\n" +
        ...
        '       <td ng-click="row.branch.expanded = !row.branch.expanded"\n' +
        '             ><a ng-click="user_clicks_branch(row.branch)"></a><span ng-if="expandingProperty.cellTemplate" class="indented tree-label" ' +
    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        attrs.iconLeaf = attrs.iconLeaf
          ? attrs.iconLeaf
          : "icon-file  glyphicon glyphicon-file  fa fa-file";
        attrs.sortedAsc = attrs.sortedAsc
          ? attrs.sortedAsc
          : "icon-file  glyphicon glyphicon-chevron-up  fa angle-up";
        attrs.sortedDesc = attrs.sortedDesc
          ? attrs.sortedDesc
          : "icon-file  glyphicon glyphicon-chevron-down  fa angle-down";
        ...
        on_treeData_change = function () {
          ...
          add_branch_to_list = function (level, branch, visible) {
            ...
            if (!branch.children || branch.children.length === 0) {
              tree_icon =
                (branch.icons && branch.icons.iconLeaf) || attrs.iconLeaf;
            } else {
              if (branch.expanded) {
                tree_icon =
                  (branch.icons && branch.icons.iconCollapse) ||
                  attrs.iconCollapse;
              } else {
                tree_icon =
                  (branch.icons && branch.icons.iconExpand) || attrs.iconExpand;
              }
            }
            branch.level = level;
            ...
            if (branch.children != null) {
              ...
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                child = _ref[_i];
                child_visible =
                  visible && (branch.expanded || branch.level < expand_level);
                _results.push(
        ...
        on_expandTo_change = function () {
          if (angular.isDefined(scope.expandTo)) {
            for_each_branch(function (b) {
              b.expanded = false;
              if (
                b[expandingProperty.field] === scope.expandTo ||
                b[expandingProperty] === scope.expandTo
              ) {
                return $timeout(function () {
...
app.provider("treegridTemplate", function () {
  var templatePath = "/js/tree/temp/template.html";
    ...
    if (!filterString || filterString.length < 3) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        if (include(item, filterString, expandingProperty, colDefinitions)) {
          for (
            var ancestorIndex = 0;
            ancestorIndex < ancestorStack.length;
            ancestorIndex++
          ) {
            ancestor = ancestorStack[ancestorIndex];
            if (ancestor.visible) {
              if (expand) ancestor.branch.expanded = true;
